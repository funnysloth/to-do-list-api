...
async def get_lists(
    ...
    if len(lists) == 0:
        return ResponseWithData(message="There are no lists.", data=[])
    lists_public = [ListPublic.model_validate(list) for list in lists]
    return ResponseWithData(message="Lists retrieved successfully", data=lists_public)


@router.get("/{list_id}", response_model=ResponseWithData)
async def get_list_by_Id(
    ...
    return ResponseWithData(message="List retrieved successfully", data=ListPublic.model_validate(found_list))
...
async def delete_list(
    ...
    return ResponseBase(message="List deleted successfully")